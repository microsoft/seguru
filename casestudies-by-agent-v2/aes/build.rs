...
fn main() {
    println!("cargo:rerun-if-changed=cuda/aes_ref.cu");
    ...
    let src = manifest.join("cuda/aes_ref.cu");
    let obj = out_dir.join("aes_ref.o");
    let lib = out_dir.join("libaes_ref.a");
    ...
    assert!(status.success(), "nvcc failed to compile cuda/aes_ref.cu");
    ...
    println!("cargo:rustc-link-lib=static=aes_ref");
