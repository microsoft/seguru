...
// operand of `#pragma unroll`, so the upstream `#pragma unroll
// BIN_KEYS_PER_THREAD` fails to compile. `_Pragma` does expand its argument.
#define DRS_PRAGMA(x) _Pragma(#x)
#define DRS_UNROLL_N(n) DRS_PRAGMA(unroll n)
    ...
    uint32_t radixShift)
{
    ...
    if (blockIdx.x < gridDim.x - 1)
    {
        DRS_UNROLL_N(BIN_KEYS_PER_THREAD)
