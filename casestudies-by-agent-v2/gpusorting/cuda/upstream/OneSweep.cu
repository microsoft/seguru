...
#ifndef G_HIST_PART_SIZE
#define G_HIST_PART_SIZE	65536
#endif
#ifndef G_HIST_VEC_SIZE
#define G_HIST_VEC_SIZE		16384
#endif

//For the digit binning
#ifndef BIN_PART_SIZE
#define BIN_PART_SIZE       7680
#endif                                    //Partition tile size in k_DigitBinning
#define BIN_HISTS_SIZE      4096                                    //Total size of warp histograms in shared memory in k_DigitBinning
#ifndef BIN_SUB_PART_SIZE
#define BIN_SUB_PART_SIZE   480
#endif                                     //Subpartition tile size of a single warp in k_DigitBinning
#ifndef BIN_WARPS
#define BIN_WARPS           16
#endif                                      //Warps per threadblock in k_DigitBinning
#ifndef BIN_KEYS_PER_THREAD
#define BIN_KEYS_PER_THREAD 15
#endif                                      //Keys per thread in k_DigitBinning
// LOCAL MODIFICATION (SeGuRu case study): nvcc 13.3 no longer macro-expands the
// operand of `#pragma unroll`. `_Pragma` does expand its argument.
#define OS_PRAGMA(x) _Pragma(#x)
#define OS_UNROLL_N(n) OS_PRAGMA(unroll n)
    ...
    uint32_t radixShift)
{
    ...
    // LOCAL MODIFICATION (SeGuRu case study): the acquired partition index gets
    // its own slot instead of being stashed in s_warpHistograms[BIN_PART_SIZE-1].
    // That stash is only safe while BIN_PART_SIZE > BIN_HISTS_SIZE; at upstream's
    // 7680 the slot is past the warp histograms, but at a 4096 tile it *is* warp
    // 15's bin 255, so the histogram increments overwrite the partition index and
    // the kernel scatters to a wild address. One extra word makes the file tunable
    // without changing what it does at upstream's own tuning.
    __shared__ uint32_t s_partitionIndexSlot;
    ...
    if (threadIdx.x == 0)
        s_partitionIndexSlot = atomicAdd((uint32_t*)&index[radixShift >> 3], 1);
    __syncthreads();
    const uint32_t partitionIndex = s_partitionIndexSlot;
    ...
    if (partitionIndex < gridDim.x - 1)
    {
        OS_UNROLL_N(BIN_KEYS_PER_THREAD)
