...
fn main() {
    println!("cargo:rerun-if-changed=cuda/heongpu_ref.cu");
    ...
    let src = manifest.join("cuda/heongpu_ref.cu");
    let obj = out_dir.join("heongpu_ref.o");
    let lib = out_dir.join("libheongpu_ref.a");

    let status = Command::new(format!("{cuda_dir}/bin/nvcc"))
        .args(["-c", src.to_str().unwrap(), "-o", obj.to_str().unwrap()])
        .args(["-O3", "-lineinfo", "-arch=native", "--compiler-options", "-fPIC"])
        .status()
        .expect("failed to invoke nvcc; set CUDA_PATH or disable the `bench` feature");
    assert!(status.success(), "nvcc failed to compile cuda/heongpu_ref.cu");
    ...
    println!("cargo:rustc-link-lib=static=heongpu_ref");
