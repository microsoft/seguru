...
# ---------------------------------------------------------------------------
# The appendix table covering every suite that has a CUDA mirror.
...
def _fmt_size(p):
    ...
    # max_pool1d uses the same window and stride at every size, so they are
    # stated in the caption instead of repeated in the widest column.
    p = p.replace("/k=4/s=4", "")
    # The suite header names what the column counts, so the cells carry no
    # variable name.
    p = p.replace("coeffs=", "")
...
# What the Size column counts, which differs per suite and is stated in the
# suite header so the cells themselves stay bare numbers.
SUITE_UNIT = {"aes": "input bytes", "heongpu": "coefficients",
...
DISPLAY = {
    ...
    "radix_upsweep": r"upsweep",
    "radix_scan": r"scan",
    "radix_downsweep": r"downsweep",
    "onesweep_global_histogram": r"global\_histogram",
    "onesweep_scan": r"onesweep scan",
    "onesweep_digit_binning": r"digit\_binning(1 \code{unsafe})",
    "onesweep_digit_binning_safe": r"digit\_binning(safe, atomic)",
}
...
def write_latex(df, rat):
    ...
    # ---- All suites with a CUDA baseline ---------------------------------
    key = ["suite", "workload", "parameter"]
    cuda = (df[(df.implementation == "cuda") & (df.variant == "stock")]
            .drop_duplicates(key).set_index(key)["value"])
    units = df.drop_duplicates(key).set_index(key)["units"]
    # onesweep_scan launches 4 blocks over the 256-bin global histogram, so it
    ...
    st["size_key"] = [cuda.loc[(r.suite, r.workload, r.parameter)]
                      for _, r in st.iterrows()]
    st["sort_name"] = [WORKLOAD_SORT.get(w, w) for w in st.workload]
    st = st.sort_values(["suite_rank", "sort_name", "size_key"])

    # The heatmap endpoints must bracket every value it colours, or
    # \ApplyGradientR asks xcolor for an out-of-range intensity. The cells show
    # two decimals, so the endpoints must bracket the rounded values.
    t_lo, t_hi = min(round(v, 2) for v in st.ratio), max(round(v, 2) for v in st.ratio)

    def suite_header(suite, cont=False):
        return (r"\multicolumn{4}{@{}l}{\textbf{" + SUITE_TITLE[suite]
                + "} (" + SUITE_UNIT[suite]
                + (r", cont.)}" if cont else r")}"))

    # Each entry is a full 4-cell row for one column block, tagged with the
    # suite it belongs to so a block that starts mid-suite can be relabelled.
    body, prev_suite = [], None
    for _, r in st.iterrows():
        if r.suite != prev_suite:
            body.append((suite_header(r.suite), r.suite))
            prev_suite = r.suite
        k = (r.suite, r.workload, r.parameter)
        u = units.loc[k]
        t = cuda.loc[k]
        # The table header declares microseconds, so times recorded in
        # milliseconds are converted rather than carrying their own unit.
        if u == "ms":
            t, u = t * 1000.0, "us"
        name = DISPLAY.get(r.workload, r.workload.replace("_", r"\_"))
        # Large microsecond values would widen the column past the text block.
        ts = f"{t:.0f}" if t >= 10000 else f"{t:.1f}"
        body.append(("{} & {} & {} & {:.2f}".format(
            name, _fmt_size(r.parameter),
            ts if u == "us" else f"{t:.1f}\\,{u}",
            r.ratio), r.suite))

    ncol = 3
    per = -(-len(body) // ncol)
    # A block that begins in the middle of a suite gets its own header, so no
    # column starts with kernel names whose suite is named in a different one.
    blocks = []
    for c in range(ncol):
        chunk = body[per * c: per * (c + 1)]
        if c and chunk and not chunk[0][0].startswith(r"\multicolumn{4}"):
            chunk.insert(0, (suite_header(chunk[0][1], cont=True), chunk[0][1]))
        blocks.append([cell for cell, _ in chunk])
    rows = max(len(b) for b in blocks)

    # An empty cell in an R column would reach \ApplyGradientR with no argument.
    filler = r"& & & \multicolumn{1}{c}{}"
    hdr = (r"Kernel & Size & CUDA & "
           r"\multicolumn{1}{c}{$\frac{\text{\sys{}}}{\text{CUDA}}$}")
    tex = [HDR,
           r"\begin{table*}[t]",
           r"\ExplSyntaxOn",
           rf"\fp_set:Nn \TZero {{{t_lo:.3f}}}",
           r"\fp_set:Nn \TOne  {1.0}",
           rf"\fp_set:Nn \TTwo  {{{t_hi:.3f}}}",
           r"\ExplSyntaxOff",
           r"\centering",
           r"\caption{Every agent-written \sys{} kernel across all five "
           r"suites, against the hand-written CUDA C++ mirror computing the "
           r"same thing (A100), "
           r"with bounds checking enabled. Cells are shaded by ratio: green is "
           r"faster than CUDA, red is slower, white is parity. Every kernel is "
           r"entirely safe Rust except the onesweep row marked "
           r"\code{unsafe}, whose global scatter writes through one "
           r"\code{unsafe} line asserting that the rank array it indexes by is "
           r"a permutation; the row above it is that same scatter written with "
           r"atomics. Rows marked \emph{(kernel)} are individual kernels of "
           r"the sorts listed above them. \code{max\_pool1d} uses a window "
           r"and stride of 4 at every size. Onesweep's fourth kernel, which "
           r"scans the 256-bin global histogram with four blocks, does the "
           r"same work at every input size and is therefore not listed."
           r"\label{tab:appendix:aibench}}",
           r"\scriptsize",
           # Tightened so the full kernel names fit the text block.
           r"\setlength{\tabcolsep}{4pt}",
           r"\begin{tabular}{@{}" + r"@{\hskip 1.1em}".join([r"llrR"] * ncol) + r"@{}}",
           r"\toprule",
           " & ".join([hdr] * ncol) + r" \\",
           " & ".join([r" & & ($\mu$s) & \multicolumn{1}{c}{}"] * ncol) + r" \\",
           r"\midrule"]
    for i in range(rows):
        cells = [b[i] if i < len(b) else filler for b in blocks]
        tex.append(" & ".join(cells) + r" \\")
    tex += [r"\bottomrule", r"\end{tabular}", r"\end{table*}", ""]
    out = PAPER / "data" / "aibench.tex"
    out.parent.mkdir(exist_ok=True)
    out.write_text("\n".join(tex))
    print("wrote", out, f"({len(body)} rows, heatmap {t_lo:.2f}..{t_hi:.2f})")
