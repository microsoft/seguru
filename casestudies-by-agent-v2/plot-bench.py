...
SUITE_ORDER = ["aes", "heongpu", "polybench", "gpusorting"]
...
def write_latex(df, rat):
    ...
    st = rat[(rat.variant == "stock") & rat.suite.isin(SUITE_ORDER)].copy()
    ...
    tex = [HDR,
           ...
           r"\ExplSyntaxOff",
           r"\centering",
           r"\caption{Every agent-written \sys{} kernel against the "
    ...
    print("wrote", PAPER / "tab_appendix_polybench.tex",
          f"({len(body)} rows, heatmap {t_lo:.2f}..{t_hi:.2f})")
