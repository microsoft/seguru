...
fn main() {
    println!("cargo:rerun-if-changed=cuda/polybench_ref.cu");
    ...
    let src = manifest.join("cuda/polybench_ref.cu");
    let obj = out_dir.join("polybench_ref.o");
    let lib = out_dir.join("libpolybench_ref.a");

    let status = Command::new(format!("{cuda_dir}/bin/nvcc"))
        .args(["-c", src.to_str().unwrap(), "-o", obj.to_str().unwrap()])
        // `-arch=native` is not optional: without it the cubin does not match
        // the device and the kernels silently fail to launch, leaving the
        // output buffers untouched.
        .args(["-O3", "-lineinfo", "-arch=native", "--use_fast_math"])
        .args(["--compiler-options", "-fPIC"])
        .status()
        .expect("failed to invoke nvcc; set CUDA_PATH or disable the `bench` feature");
    assert!(status.success(), "nvcc failed to compile cuda/polybench_ref.cu");
    ...
    println!("cargo:rustc-link-lib=static=polybench_ref");
