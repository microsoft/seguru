...
use rustc_codegen_ssa_gpu::MemFlags;
...
impl<'ll, 'tcx> FnAbiLlvmExt<'ll, 'tcx> for FnAbi<'tcx, Ty<'tcx>> {
    fn llvm_type(&self, cx: &CodegenCx<'ll, 'tcx>) -> &'ll Type {
        ...
        for arg in args {
            ...
            // code all over the place, both in the codegen_llvm and rustc_codegen_ssa_gpu crates. That's how
    ...
    fn apply_attrs_llfn(
        ...
        if let Conv::RiscvInterrupt { kind } = self.conv {
            func_attrs.push(llvm::CreateAttrStringValue(
                cx.llcx,
                "interrupt",
                kind.as_str(),
            ));
        ...
        match &self.ret.mode {
            ...
            PassMode::Cast { cast, pad_i32: _ } => {
                cast.attrs
                    .apply_attrs_to_llfn(llvm::AttributePlace::ReturnValue, cx, llfn);
            }
            _ => {}
        }
    ...
    fn apply_attrs_callsite(&self, bx: &mut Builder<'_, 'll, 'tcx>, callsite: &'ll Value) {
        ...
        match &self.ret.mode {
            ...
            PassMode::Indirect {
                ...
                let sret = llvm::CreateStructRetAttr(
                    bx.cx.llcx,
                    bx.cx
                        .type_array(bx.cx.type_i8(), self.ret.layout.size.bytes()),
                );
        ...
        if element_type_index >= 0 {
            ...
            let pointee_ty = arg_ty
                .builtin_deref(true)
                .expect("Must be pointer argument");
