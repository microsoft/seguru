...
use rustc_codegen_ssa_gpu::back::archive::{
...
impl<'a> ArchiveBuilder for LlvmArchiveBuilder<'a> {
    ...
    fn build(mut self: Box<Self>, output: &Path) -> bool {
        match self.build_with_llvm(output) {
            ...
            Err(error) => self.sess.dcx().emit_fatal(ArchiveBuildFailure {
...
fn get_llvm_object_symbols(
    ...
    if err.is_null() {
        ...
        if buf.starts_with(&[0xDE, 0xCE, 0x17, 0x0B]) || buf.starts_with(&[b'B', b'C', 0xC0, 0xDE])
        {
            ...
            eprintln!(
                "warning: Failed to read symbol table from LLVM bitcode: {}",
                error
            );
            return Ok(true);
        } else {
