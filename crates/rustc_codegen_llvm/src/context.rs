...
use rustc_codegen_ssa_gpu::base::{wants_msvc_seh, wants_wasm_eh};
use rustc_codegen_ssa_gpu::errors as ssa_errors;
...
pub(crate) unsafe fn create_module<'ll>(
    ...
    if matches!(reloc_model, RelocModel::Pic | RelocModel::Pie) {
        ...
        if reloc_model == RelocModel::Pie
            || tcx
                .crate_types()
                .iter()
                .all(|ty| *ty == CrateType::Executable)
        {
            unsafe {
    ...
    if let Some(BranchProtection { bti, pac_ret }) = sess.opts.unstable_opts.branch_protection {
        if sess.target.arch == "aarch64" {
            ...
            let pac_opts = pac_ret.unwrap_or(PacRet {
                leaf: false,
                pc: false,
                key: PAuthKey::A,
            });
...
impl<'ll, 'tcx> CodegenCx<'ll, 'tcx> {
    pub(crate) fn new(
        ...
        let coverage_cx = tcx
            .sess
            .instrument_coverage()
            .then(coverageinfo::CguCoverageContext::new);
    ...
    pub(crate) fn coverage_cx(&self) -> &coverageinfo::CguCoverageContext<'ll, 'tcx> {
        self.coverage_cx
            .as_ref()
            .expect("only called when coverage instrumentation is enabled")
    }
...
impl<'ll> CodegenCx<'ll, '_> {
    ...
    fn declare_intrinsic(&self, key: &str) -> Option<(&'ll Type, &'ll Value)> {
        ...
        ifn!(
            ...
            fn(ptr, t_i32, t_metadata) -> mk_struct! {ptr, i1}
        );
