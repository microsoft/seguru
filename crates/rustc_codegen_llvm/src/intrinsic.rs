...
use rustc_codegen_ssa_gpu::base::{compare_simd_types, wants_msvc_seh, wants_wasm_eh};
use rustc_codegen_ssa_gpu::common::{IntPredicate, TypeKind};
use rustc_codegen_ssa_gpu::errors::{ExpectedPointerMutability, InvalidMonomorphization};
use rustc_codegen_ssa_gpu::mir::operand::{OperandRef, OperandValue};
use rustc_codegen_ssa_gpu::mir::place::{PlaceRef, PlaceValue};
...
impl<'ll, 'tcx> IntrinsicCallBuilderMethods<'tcx> for Builder<'_, 'll, 'tcx> {
    fn codegen_intrinsic_call(
        ...
        let llval = match name {
            ...
            _ => {
                debug!(
                    "unknown intrinsic '{}' -- falling back to default body",
                    name
                );
...
fn codegen_msvc_try<'ll>(
    ...
    let (llty, llfn) = get_rust_try_fn(bx, &mut |mut bx| {
        ...
        let type_info = bx.const_struct(
            &[type_info_vtable, bx.const_null(bx.type_ptr()), type_name],
            false,
        );
...
fn generic_simd_intrinsic<'ll, 'tcx>(
    ...
    macro_rules! require_simd {
        ($ty: expr, $variant:ident) => {{
            require!(
                $ty.is_simd(),
                InvalidMonomorphization::$variant {
                    span,
                    name,
                    ty: $ty
                }
    ...
    macro_rules! require_int_ty {
        ($ty: expr, $diag: expr) => {
            match $ty {
                ty::Int(i) => i
                    .bit_width()
                    .unwrap_or_else(|| bx.data_layout().pointer_size.bits()),
                _ => {
                    return_error!($diag);
                }
            }
    ...
    fn vector_mask_to_bitmask<'a, 'll, 'tcx>(
        ...
        let shift_idx = bx
            .cx
            .const_int(bx.type_ix(in_elem_bitwidth), (in_elem_bitwidth - 1) as _);
        ...
        let indices: Option<Vec<_>> = idx
            ...
            .map(|(arg_idx, val)| {
                ...
                if idx >= i32::try_from(total_len).unwrap() {
                    bx.sess()
                        .dcx()
                        .emit_err(InvalidMonomorphization::SimdIndexOutOfBounds {
                            span,
                            name,
    ...
    if name == sym::simd_shuffle {
        ...
        let n: u64 = if idx_ty.is_simd()
            ...
            return_error!(InvalidMonomorphization::SimdShuffle {
                span,
                name,
                ty: idx_ty
            })
    ...
    if name == sym::simd_select {
        ...
        let in_elem_bitwidth = require_int_ty!(
            m_elem_ty.kind(),
            InvalidMonomorphization::MaskType {
                span,
                name,
                ty: m_elem_ty
            }
    ...
    if name == sym::simd_bitmask {
        ...
        let in_elem_bitwidth = require_int_or_uint_ty!(
            in_elem.kind(),
            InvalidMonomorphization::VectorArgument {
                span,
                name,
                in_ty,
                in_elem
    ...
    fn llvm_vector_str(bx: &Builder<'_, '_, '_>, elem_ty: Ty<'_>, vec_len: u64) -> String {
        match *elem_ty.kind() {
            ty::Int(v) => format!(
                ...
                v.normalize(bx.target_spec().pointer_width)
                    .bit_width()
                    .unwrap()
            ),
            ty::Uint(v) => format!(
    ...
    if name == sym::simd_masked_load {
        ...
        let fn_ty = bx.type_func(
            &[llvm_pointer, alignment_ty, mask_ty, llvm_elem_vec_ty],
            llvm_elem_vec_ty,
        );
        let f = bx.declare_cfn(&llvm_intrinsic, llvm::UnnamedAddr::No, fn_ty);
        let v = bx.call(
    ...
    if name == sym::simd_masked_store {
        ...
        let fn_ty = bx.type_func(
            &[llvm_elem_vec_ty, llvm_pointer, alignment_ty, mask_ty],
            ret_t,
        );
        let f = bx.declare_cfn(&llvm_intrinsic, llvm::UnnamedAddr::No, fn_ty);
        let v = bx.call(
    ...
    if name == sym::simd_cast_ptr {
        ...
        match in_elem.kind() {
            ...
            _ => {
                return_error!(InvalidMonomorphization::ExpectedPointer {
                    span,
                    name,
                    ty: in_elem
                })
    ...
    if name == sym::simd_expose_provenance {
        ...
        match out_elem.kind() {
            ...
            _ => return_error!(InvalidMonomorphization::ExpectedUsize {
                span,
                name,
                ty: out_elem
            }),
    ...
    if name == sym::simd_arith_offset {
        ...
        if !matches!(
            offsets_elem.kind(),
            ty::Int(ty::IntTy::Isize) | ty::Uint(ty::UintTy::Usize)
        ) {
            span_bug!(
                span,
