...
__global__ void innerProductKernel(float *A, int64_t a_len, float *B,
                                   int64_t b_len, float *C, int64_t c_len,
                                   int64_t c_w, int64_t N) {
...
int main(int argc, char **argv) {
  ...
  innerProductKernel<<<blocksPerGrid, threadsPerBlock>>>(d_A, N, d_B, N, d_C, N,
                                                         1, N);
  cudaDeviceSynchronize();
