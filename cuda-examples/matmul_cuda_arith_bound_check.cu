    ...
    for (i = 0; i < N / BLOCK_SIZE / blockDim.y; i++) {
        ...
        for (j = 0; j < N / BLOCK_SIZE / blockDim.x; j++) {
            if(row < N && col < N) {
                ...
                int64_t b_idx = col;
                for(int k = 0; k < N; ++k) {
                    long long a_mask = ((long long)(a_idx - total_size) >> (long long)63);
                    *(int *)((int64_t)(&dummy) & a_mask) = 1;

                    
                    long long b_mask = ((long long)(b_idx - total_size) >> (long long)63);
                    *(int *)((int64_t)(&dummy) & b_mask) = 1;
                ...
                long long mask = ((long long)(c_idx - total_size) >> (long long)63);
                *(int *)((int64_t)(&dummy) & mask) = 1;
                C[c_idx] = sum;
            }
