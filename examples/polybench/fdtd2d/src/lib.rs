...
#[gpu::cuda_kernel]
pub fn fdtd_step1(fict: &[f32], ey: &mut [f32], hz: &[f32], nx: usize, ny: usize, t: usize) {
...
#[gpu::cuda_kernel]
pub fn fdtd_step2(ex: &mut [f32], hz: &[f32], nx: usize, ny: usize) {
...
#[gpu::cuda_kernel]
pub fn fdtd_step3(ex: &[f32], ey: &[f32], hz: &mut [f32], nx: usize, ny: usize) {
    ...
    fn run_fdtd2d(
        ...
        cuda_ctx(0, |ctx, m| {
            ...
            for t in 0..tmax {
                ...
                fdtd_step1::launch(c1, ctx, m, &d_fict, &mut d_ey, &d_hz, nx, ny, t)
                    .expect("step1");
                let c2 = gpu_host::gpu_config!(grid_x, grid_y, 1, block_size, block_size, 1, 0);
                fdtd_step2::launch(c2, ctx, m, &mut d_ex, &d_hz, nx, ny).expect("step2");
                let c3 = gpu_host::gpu_config!(grid_x, grid_y, 1, block_size, block_size, 1, 0);
                fdtd_step3::launch(c3, ctx, m, &d_ex, &d_ey, &mut d_hz, nx, ny)
                    .expect("step3");
            }
